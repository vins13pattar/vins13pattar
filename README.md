<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,100:1f6feb&height=180&section=header&text=Vinod%20S%20Pattar&fontSize=42&fontColor=ffffff&animation=fadeIn&desc=Principal%20Engineer%20%C2%B7%20Full-Stack%20Architect%20%C2%B7%20GenAI&descSize=16&descAlignY=65" width="100%"/>

[![Website](https://img.shields.io/badge/vinodspattar.in-0d1117?style=for-the-badge&logo=googlechrome&logoColor=white)](https://vinodspattar.in)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/vins13pattar/)
[![X](https://img.shields.io/badge/@vins13pattar-000000?style=for-the-badge&logo=x&logoColor=white)](https://x.com/vins13pattar)

</div>

## 👋 About Me

I'm a **Principal Engineer at Tharsha Ventures**, where I lead engineering for an elder-technology ecosystem — five products spanning community, talent, housing, wealth, and life-planning for India's senior citizens.

- 🏗️ **Building** — EldersIndia · EldersTalent · EldersAbode · EldersWealth · LifeArc
- 🎓 **Teaching** — Generative AI curriculum at MicroDegree (Bengaluru & Mangaluru)
- 🤖 **Exploring** — AI agents, LangChain, agentic coding workflows & compliance automation
- 🇮🇳 **Based in** — Bengaluru, India

## 🚀 Featured Projects

### 🛰️ Flagship Open Source

| Project | Description |
|---|---|
| [**trackflow-open-source**](https://github.com/vins13pattar/trackflow-open-source) | Multi-tenant GPS tracking SaaS — in-house GT06/H02/Teltonika protocol decoders, real-time MapLibre dashboard, Postgres RLS tenant isolation, Expo mobile tracker. Apache-2.0 |

### 🤖 AI Agents & GenAI Systems

| Project | Description |
|---|---|
| [**multi-agent-knowledge-assisant**](https://github.com/vins13pattar/multi-agent-knowledge-assisant) | Enterprise-grade RAG + multi-agent system — CrewAI, LangGraph, Streamlit, fully Dockerized |
| [**feedback-intelligence**](https://github.com/vins13pattar/feedback-intelligence) | Multi-provider LLM system turning raw customer feedback into structured intelligence |
| [**code-debugger-assistant**](https://github.com/vins13pattar/code-debugger-assistant) | Terminal-based AI debugging assistant — LangChain 1.0 (LCEL) + Textual TUI |
| [**n8n-smart-inbox-agent**](https://github.com/vins13pattar/n8n-smart-inbox-agent) | AI inbox triage on n8n — classifies Telegram/Gmail messages, drafts replies with human approval |
| [**dpdpa-compliance**](https://github.com/vins13pattar/dpdpa-compliance) | DPDPA 2023 compliance skill for AI coding agents — audit, implement, remediate |

### 📱 Mobile & Product

| Project | Description |
|---|---|
| [**watery**](https://github.com/vins13pattar/watery) | Gamified water-intake tracker in Expo React Native — streaks, achievements, weather-aware insights |
| [**Exotel-WhatsApp**](https://github.com/vins13pattar/Exotel-WhatsApp) ⭐5 | Dockerized reference implementation for Exotel WhatsApp Business APIs — REST backend + admin UI |
| [**v0-ai-use-case-generator**](https://github.com/vins13pattar/v0-ai-use-case-generator) | Stateless Next.js app generating personalized GenAI use cases from your professional profile |

### 🛠️ Developer Tools & Open Source

| Project | Description |
|---|---|
| [**gh-stars-organizer**](https://github.com/vins13pattar/gh-stars-organizer) ⭐8 | Organize GitHub stars into intelligent lists using LLMs & embeddings |
| [**gen-ai-interview-preparation**](https://github.com/vins13pattar/gen-ai-interview-preparation) | Free, open-source, self-hosted GenAI interview prep tool |

### 🎓 Teaching & Learning Resources

| Project | Description |
|---|---|
| [**Langchain-Byte-sized-lessons**](https://github.com/vins13pattar/Langchain-Byte-sized-lessons) | Byte-sized interactive HTML course covering official LangChain v1 docs |
| [**langchain-python**](https://github.com/vins13pattar/langchain-python) | Structured collection of LangChain examples |
| [**Python-for-Gen-AI**](https://github.com/vins13pattar/Python-for-Gen-AI) ⭐3 | Hands-on workspace: Python fundamentals → GenAI building blocks |

<!-- WEBINAR REPO — replace REPO_NAME and the description, then move this row into the table above:
| [**REPO_NAME**](https://github.com/vins13pattar/REPO_NAME) | Webinar platform — description here |
-->

## 🛠️ Tech Stack

<div align="center">

<a href="https://github.com/vins13pattar">
  <img src="https://skillicons.dev/icons?i=py,ts,react,nextjs,laravel,fastapi,aws,docker,postgres,redis&perline=10" alt="Core tech stack"/>
</a>

![Expo](https://img.shields.io/badge/Expo-000020?style=flat-square&logo=expo&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white)
![LangGraph](https://img.shields.io/badge/LangGraph-1C3C3C?style=flat-square&logo=langgraph&logoColor=white)
![n8n](https://img.shields.io/badge/n8n-EA4B71?style=flat-square&logo=n8n&logoColor=white)
![Claude](https://img.shields.io/badge/Claude_AI-D97757?style=flat-square&logo=anthropic&logoColor=white)

</div>

## 📊 GitHub Stats

<div align="center">

<a href="https://github.com/vins13pattar">
  <img src="https://github-readme-stats-delta-eight-60.vercel.app/api?username=vins13pattar&show_icons=true&theme=github_dark&hide_border=true&bg_color=0d1117&rank_icon=github" height="165" alt="GitHub stats"/>
</a>
<a href="https://github.com/vins13pattar">
  <img src="https://github-readme-stats-delta-eight-60.vercel.app/api/top-langs/?username=vins13pattar&layout=compact&theme=github_dark&hide_border=true&bg_color=0d1117&langs_count=8" height="165" alt="Top languages"/>
</a>

<a href="https://github.com/vins13pattar">
  <img src="https://github-readme-streak-stats-self-xi.vercel.app/?user=vins13pattar&theme=github-dark-blue&hide_border=true&background=0d1117" alt="Streak stats"/>
</a>

<a href="https://github.com/vins13pattar">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=vins13pattar&bg_color=0d1117&color=58a6ff&line=1f6feb&point=58a6ff&area=true&hide_border=true" width="95%" alt="Contribution activity graph"/>
</a>

</div>

## 🐍 Contribution Snake

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/vins13pattar/vins13pattar/output/github-snake-dark.svg"/>
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/vins13pattar/vins13pattar/output/github-snake.svg"/>
  <img src="https://raw.githubusercontent.com/vins13pattar/vins13pattar/output/github-snake.svg" alt="Contribution snake animation"/>
</picture>

</div>

## 🤝 Let's Connect

Open to conversations about **GenAI applications, elder-tech, compliance automation, and engineering leadership**. Reach me on [LinkedIn](https://www.linkedin.com/in/vins13pattar/) or via [vinodspattar.in](https://vinodspattar.in).

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1f6feb,100:0d1117&height=100&section=footer" width="100%"/>

</div>
